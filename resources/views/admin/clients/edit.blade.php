...
@section('page_title', __('admin.edit_client'))
    ...
    <form action="{{ route('admin.clients.update', $client) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6">
            ...
            <div>
                ...
                <div class="space-y-4">
                    @if($client->logo)
                        <div class="w-24 h-24 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 p-2 bg-white dark:bg-slate-900">
                            <img src="{{ asset('storage/' . $client->logo) }}" class="w-full h-full object-contain">
                        </div>
                    @endif
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $client->order) }}" required
                ...
                <div class="flex items-center gap-3 pt-6">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $client->is_active) ? 'checked' : '' }} class="w-5 h-5 text-brand-500 border-slate-300 rounded-lg focus:ring-brand-500 cursor-pointer">
