...
@section('title', __('admin.add') . ' ' . __('admin.testimonials'))
@section('page_title', __('admin.add') . ' ' . __('admin.testimonials'))
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div id="content-en" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="client_name[en]" value="{{ old('client_name.en') }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="client_role[en]" value="{{ old('client_role.en') }}" required
                    ...
                    <div>
                        ...
                        <textarea name="content[en]" rows="4" required
                            placeholder="Client's testimonial content..."
                ...
                <div id="content-ar" class="space-y-6 hidden" dir="rtl">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="client_name[ar]" value="{{ old('client_name.ar') }}" required
                                placeholder="مثال: جين سميث"
                        ...
                        <div>
                            ...
                            <input type="text" name="client_role[ar]" value="{{ old('client_role.ar') }}" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6 text-start">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{{ __('admin.photo') }} ({{ __('admin.optional') ?? 'Optional' }})</label>
                    ...
                    <div>
                        ...
                        <select name="rating" class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border @error('rating') border-red-500 @else border-slate-200 dark:border-slate-700 @enderror rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all dark:text-white">
                            @for($i=5; $i>=1; $i--)
                                <option value="{{ $i }}" {{ old('rating', 5) == $i ? 'selected' : '' }}>{{ $i }} {{ __('admin.stars') }}</option>
                            @endfor
                        </select>
                        @error('rating')
                            <p class="mt-1 text-xs text-red-500 font-bold flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
        ...
        <div class="flex items-center gap-4">
            <button type="submit" class="px-8 py-4 bg-brand-500 hover:bg-brand-600 text-white rounded-2xl font-bold transition-all shadow-lg shadow-brand-500/25 transform hover:-translate-y-1">
                {{ __('admin.add') }} {{ __('admin.testimonials') }}
            </button>
