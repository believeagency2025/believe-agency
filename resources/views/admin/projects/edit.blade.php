...
@section('title', __('admin.edit_project') . ': ' . ($project->title[app()->getLocale()] ?? $project->title['en'] ?? 'Untitled'))
@section('page_title', __('admin.edit_project'))
            ...
            <div class="p-8">
                ...
                <div id="content-en" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title[en]" value="{{ old('title.en', $project->title['en'] ?? '') }}" required
                ...
                <div id="content-ar" class="space-y-6 hidden" dir="rtl">
                    <div>
                        ...
                        <input type="text" name="title[ar]" value="{{ old('title.ar', $project->title['ar'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <textarea name="description[ar]" rows="3"
                            ...
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border @error('description.ar') border-red-500 @else border-slate-200 dark:border-slate-700 @enderror rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all dark:text-white">{{ old('description.ar', $project->description['ar'] ?? '') }}</textarea>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="completion_date" value="{{ old('completion_date', $project->completion_date ? $project->completion_date->format('Y-m-d') : '') }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="order" value="{{ old('order', $project->order) }}" required
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }} class="w-5 h-5 text-brand-500 border-slate-300 rounded-lg focus:ring-brand-500 cursor-pointer">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6">
                ...
                <div>
                    ...
                    @if($project->main_image)
                        <div class="mb-4 relative w-32 h-20 rounded-xl overflow-hidden shadow-sm">
                            <img src="{{ asset('storage/' . $project->main_image) }}" class="w-full h-full object-cover">
                        </div>
                    @endif
                    ...
                    @error('main_image')
                        <p class="mt-1 text-xs text-red-500 font-bold flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
                    <p class="text-[10px] text-slate-500 mt-2">{{ __('admin.recommended_size') }}: 1200x800px. {{ __('admin.max_2mb') }}. {{ __('admin.keep_empty_hint') ?? 'Leave empty to keep current.' }}</p>
                </div>
                ...
                <div>
                    ...
                    @if($project->gallery)
                        <div class="grid grid-cols-4 gap-3 mb-4">
                            @foreach($project->gallery as $image)
                                <div class="relative w-full h-16 rounded-lg overflow-hidden shadow-sm">
                    ...
                    @error('gallery.*')
                        <p class="mt-1 text-xs text-red-500 font-bold flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
                    <p class="text-[10px] text-slate-500 mt-2">{{ __('admin.new_gallery_hint') ?? 'New gallery images will be added to existing ones.' }}</p>
                </div>
            </div>
        </div>
