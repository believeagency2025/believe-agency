...
@section('page_title', __('admin.create_category'))
...
<div class="max-w-2xl">
    <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border @error('type') border-red-500 @else border-slate-200 dark:border-slate-700 @enderror rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all dark:text-white">
                        <option value="project">{{ __('admin.projects') }}</option>
                        <option value="service">{{ __('admin.services') }}</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-xs text-red-500 font-bold flex items-center gap-1"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
        ...
        <div class="flex items-center gap-4">
            <button type="submit" class="px-8 py-4 bg-brand-500 hover:bg-brand-600 text-white rounded-2xl font-bold transition-all shadow-lg shadow-brand-500/25 transform hover:-translate-y-1">{{ __('admin.save_category') }}</button>
