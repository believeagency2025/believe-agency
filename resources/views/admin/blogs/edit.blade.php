...
@section('title', __('admin.edit_blog') ?? 'Edit Blog')
@section('page_title', __('admin.edit_blog') ?? 'Edit Blog')
    ...
    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            ...
            <div class="p-8">
                @php
                    ...
                    $contentTranslations = $blog->getTranslations('content');
                @endphp
                <!-- English Content -->
                <div id="content-en" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title[en]" value="{{ old('title.en', $titleTranslations['en'] ?? '') }}" required
                ...
                <div id="content-ar" class="space-y-6 hidden" dir="rtl">
                    <div>
                        ...
                        <input type="text" name="title[ar]" value="{{ old('title.ar', $titleTranslations['ar'] ?? '') }}" required
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-4">
                        <div id="image-preview" class="w-24 h-24 rounded-2xl bg-slate-100 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 overflow-hidden">
                            @if($blog->image)
                                <i class="fas fa-image text-2xl hidden" id="image-placeholder"></i>
                                <img src="{{ asset('storage/' . $blog->image) }}" id="image-element" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="flex-1">
                            ...
                            <label for="image-input" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-xl font-bold cursor-pointer transition-all">
                                ...
                                {{ __('admin.change_photo') ?? 'Change Photo' }}
                            </label>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $blog->order) }}" required
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_active" id="is_active" {{ $blog->is_active ? 'checked' : '' }} value="1" class="w-5 h-5 text-brand-500 border-slate-300 rounded-lg focus:ring-brand-500 cursor-pointer">
        ...
        <div class="flex items-center gap-4">
            <button type="submit" class="px-8 py-4 bg-brand-500 hover:bg-brand-600 text-white rounded-2xl font-bold transition-all shadow-lg shadow-brand-500/25 transform hover:-translate-y-1">
                {{ __('admin.save_changes') ?? 'Save Changes' }}
            </button>
